...
<head>
    ...
    <!-- AMS popup referee: one client-side coin flip decides which popup system may
         fire this pageview - Damrei's underlay (the two zones wrapped below) or the
         MSA popup plugin (AMS MSA Popup, which reads this same flag). Mobile only:
         MSA zone 94 never fires on desktop, so desktop always resolves to "damrei"
         and desktop behaviour is unchanged. Client-side because the page is cached.
         Design record: frontend repo, docs/wp-ads/README.md section 12. -->
    <script>
        window.AMS_POPUP_WINNER = (/Mobi|Android|iPhone|iPad|iPod/.test(navigator.userAgent) && Math.random() < 0.5) ? "msa" : "damrei";
    </script>
    <!--GPAS-->
    ...
    <script>
        gammatag.cmd.push(function() {
            ...
            /* Damrei - Underlay -- AMS referee: skipped when MSA won the pageview.
               A zone never defined is never requested, so no billed impression.
               "!== msa" (not "=== damrei") so a missing flag fails open to today's
               behaviour instead of silencing the underlay. */
            if (window.AMS_POPUP_WINNER !== "msa") {
            gammatag.defineZone({
            ...
            /* Damrei - Underlay - 2 -- AMS referee: same rule as Underlay above. */
            if (window.AMS_POPUP_WINNER !== "msa") {
            gammatag.defineZone({
