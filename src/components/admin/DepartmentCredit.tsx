...
const SIZE = {
  ...
  coordinator: css({ width: { base: "104px", sm: "122px" }, height: { base: "104px", sm: "122px" } }),
...
const NAME = {
  ...
  coordinator: css({ fontSize: { base: "15px", sm: "16.5px" }, fontWeight: 800, letterSpacing: "-0.01em", lineHeight: 1.2 }),
  developer: css({ fontSize: { base: "14.5px", sm: "15.5px" }, fontWeight: 800, letterSpacing: "-0.01em", lineHeight: 1.2 }),
} as const;
  ...
  return (
    <div className={overlay} onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="dept-title" onClick={(event) => event.stopPropagation()} className={panel}
        ...
        <Spine delay={360} />
        <div className={fan}>
          <div className={cx(fanBar, tier)} style={{ background: LINE_H, boxShadow: LINE_GLOW, animationDelay: "400ms" }} />
          <div className={teamGrid}>
            {DEVELOPERS.map((person, index) => (
              <div key={person.name} className={teamCol}>
                <span className={cx(stub, tier)} style={{ background: LINE_V, boxShadow: LINE_GLOW, animationDelay: `${440 + index * 70}ms` }} />
                <Portrait person={person} level="developer" delay={480 + index * 70} />
              </div>
            ))}
          </div>
        </div>
